import { db, usersTable, leavesTable, outpassesTable, gateLogsTable, departmentsTable, classesTable } from "@workspace/db";
import { ALL_COLLEGE_DEPARTMENTS } from "@workspace/db";
...
export async function populateRealisticTestData() {
  logger.info("Starting Realistic Test Data Population (20 Students + JKKM ID Cards)...");
  ...
  // 4. 20 Realistic Student Profiles
  const studentsData = [
  ...
  if (vimalId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, vimalId));
    if (!existingLeave) {
      ...
      const [existingOutpass] = await db.select().from(outpassesTable).where(
        or(eq(outpassesTable.outpassCode, "OUT-MECH-25001"), eq(outpassesTable.gatePassNumber, "GP-2026-0001"))
      );
      if (!existingOutpass) {
        await db.insert(outpassesTable).values({
          ...
          status: "returned",
          exitTime: new Date(Date.now() - 86400000 * 2 + 10 * 3600000),
          returnTime: new Date(Date.now() - 86400000 * 2 + 18 * 3600000),
      ...
      await db.insert(gateLogsTable).values([
        {
          ...
          confidenceScore: 100,
          leaveId: Number(leaveId1),
          gateName: "Main Gate 1 (ID Barcode: 25ME029)",
  ...
  if (azhagesanId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, azhagesanId));
    if (!existingLeave) {
  ...
  if (chinrajId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, chinrajId));
    if (!existingLeave) {
      ...
      const [existingOutpass3] = await db.select().from(outpassesTable).where(
        or(eq(outpassesTable.outpassCode, "OUT-AUTO-25002"), eq(outpassesTable.gatePassNumber, "GP-2026-0002"))
      );
      if (!existingOutpass3) {
        await db.insert(outpassesTable).values({
          ...
          status: "verified",
          exitTime: new Date(Date.now() - 3600000 * 2),
          gateLocation: "Main Gate 1",
          approvedByWarden: "Mr. R. Murugan",
          approvedByTutor: "Prof. S. Soundar",
          approvedByHod: "Dr. P. Ramesh",
  ...
  if (karthickId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, karthickId));
    if (!existingLeave) {
      await db.insert(leavesTable).values({
  ...
  if (kavinId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, kavinId));
    if (!existingLeave) {
      await db.insert(leavesTable).values({
        studentId: kavinId,
        passType: "hostel_leave",
  ...
  if (dineshId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, dineshId));
    if (!existingLeave) {
      await db.insert(leavesTable).values({
        studentId: dineshId,
        passType: "hostel_leave",
  ...
  if (priyaId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, priyaId));
    if (!existingLeave) {
      await db.insert(leavesTable).values({
        studentId: priyaId,
        passType: "hostel_leave",
  ...
  if (vigneshId) {
    const [existingLeave] = await db.select().from(leavesTable).where(eq(leavesTable.studentId, vigneshId));
    if (!existingLeave) {
      await db.insert(leavesTable).values({
        studentId: vigneshId,
        passType: "hostel_leave",
  ...
  logger.info("20 Realistic Students & Workflow Data initialized successfully!");
